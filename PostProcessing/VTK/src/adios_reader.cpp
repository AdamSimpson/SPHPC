...
template<typename T>
std::vector<T> AdiosReader::FetchValue(const std::string& value_name) {
  ...
  adios_schedule_read(adios_file, adios_selection, value_name.c_str(), 0, 1, value_data.data());
...
// Explicitly instantiate double reader
template std::vector<double> AdiosReader::FetchValue<double>(const std::string&);
